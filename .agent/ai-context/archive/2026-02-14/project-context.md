...
기준일: 2026-02-12 (KST)
프로젝트명: TailLog
현재 단계: Phase 7 (AI Recommendation 시스템 고도화) 진행 중
...
2. 7단계 온보딩 설문 제출 (게스트: anonymous_sid 쿠키 기반)
...
- 완료:
  ...
  - **AI Recommendation 시스템 정책 강화** (Admin 전용 비용 조회, Feedback Enum, window_days 제약)
- 진행:
  - AI 코칭 고도화(RAG 연계 준비)
  - AI 비용 최적화 및 예산 대시보드 (Phase 7)
...
- 게스트 UX: 설문→결과(partial)까지 로그인 없이 접근 가능
- 라우트 정축화: `/survey` canonical (구 `/Survey`, `/checkup` 폐기)
