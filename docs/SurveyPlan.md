...
테일로그(TailLog)'의 브랜드 아이덴티티와 랜딩페이지의 전략적 흐름을 온보딩 설문(/survey)에 그대로 녹여내는 것은 유저의 이탈을 막고 신뢰를 형성하는 데 매우 중요합니다.
...
*   **Alive Aesthetic**: 유기적 배경 블롭(Organic Blobs), 고도화된 글래스모피즘 컨테이너 적용.
*   **UX Geometry**: 설문 조사 컨테이너 **720px 고정 높이** 설정을 통해 레이아웃 점프 방지 및 각 단계의 정보 가독성 최적화.
*   **고단련된 스크롤**: `no-scrollbar` 유틸리티를 활용한 내부 스크롤 시스템으로 풍부한 내용을 깔끔한 프레임 안에서 제공.
*   **초개인화 마이크로 카피**: 유저가 입력한 강아지 이름(예: "머루")을 설문 질문에 지속 노출하며 시네마틱 애니메이션으로 전환.
*   **Lottie 활용**: 질문 성격에 맞는 애니메이션 배치.

## 3. 이탈 방지를 위한 UX 장치
*   **대화형 프로그레스 바**: 목표 지점으로 이동하는 시각적 피드백 (AnimatedCounter 연동).
*   **글로벌 내비게이션**: 개발 단계에서 빠른 테스트를 위한 **디버그 내비게이션** 및 Skip 로직 지원.
